...
namespace toolkit
{
    ...
    int Commands::doctor(const std::filesystem::path &workspaceRoot, bool sdkDetails)
    {
        ...
        LogInfo("ToolkitBuild doctor");
        LogInfo("workspace: {}", workspaceRoot.string());

        const bool hasManifest = File::exists(CliWorkspace::manifest(workspaceRoot));
        LogInfo("tbuild.deps.lua: {}", hasManifest ? "found" : "missing");
        ok &= hasManifest;

        std::string error;
        const bool tkbWritable = File::mkdir(workspaceRoot / ".tbuild", &error);
        LogInfo(".tbuild: {}", tkbWritable ? "writable" : ("error: " + error));
        ok &= tkbWritable;
