                           ...
                           const std::vector<std::string> &cliOverrides)
    {
        ...
        for (const auto &package : lockFile.packages)
        {
            ...
            if (package.sourceType == "archive")
            {
                ...
                cachePath = workspaceRoot / ".tbuild" / "packages" / "_archives" / Path::sanitizePackagePath(package.name) / Path::sanitizePackagePath(key) / "src";
            }
            const auto artifactPath = workspaceRoot / ".tbuild" / "artifacts" / package.artifactId;
