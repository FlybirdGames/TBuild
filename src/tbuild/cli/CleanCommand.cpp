...
namespace toolkit
{
                     ...
                     const std::string &packageName)
    {
        ...
        {
            diagnostics.error("clean --all already removes .tbuild; do not combine it with --artifacts, --sources, or a package name");
            return 1;
        }

        const auto tbuildRoot = workspaceRoot / ".tbuild";
        ...
        const auto normalizedTbuildRoot = (normalizedWorkspaceRoot / ".tbuild").lexically_normal();
        auto isAllowedCleanPath = [&](const std::filesystem::path &path) -> bool
        {
            ...
            const auto lowerTbuildRoot = StringUtils::toLower(normalizedTbuildRoot.string());
            if (lowerPath == lowerTbuildRoot)
            {
                return true;
            }

            auto lowerPrefix = lowerTbuildRoot;
        ...
        auto removePath = [&](const std::filesystem::path &path) -> bool
        {
            ...
            {
                diagnostics.error("refusing to clean path outside .tbuild: " + path.string());
                return false;
            }
            std::error_code ec;
        ...
        if (all)
        {
            ok &= removePath(tbuildRoot);
        }
        else
        {
            if (!packageName.empty())
            {
                ok &= removePath(tbuildRoot / "build-packages" / Path::sanitizePackagePath(packageName));
                if (artifacts)
                {
                    ok &= removePath(tbuildRoot / "artifacts" / Path::sanitizePackagePath(packageName));
                }
                if (sources)
                {
                    ok &= removePath(tbuildRoot / "packages" / Path::sanitizePackagePath(packageName));
                }
            }
            else
            {
                ok &= removePath(tbuildRoot / "generated");
                ok &= removePath(tbuildRoot / "tmp");
                ok &= removePath(tbuildRoot / "build-packages");
                if (artifacts)
                {
                    ok &= removePath(tbuildRoot / "artifacts");
                }
                if (sources)
                {
                    ok &= removePath(tbuildRoot / "packages");
                }
            }
        }
        if (lock)
