...
TEST(ArchiveTests, FetcherReusesExtractedArchiveWhenMarkerMatches)
{
    ...
    toolkit::JsonDiagnosticSink diagnostics;
    ASSERT_TRUE(fetcher.fetchAndExtract(dep, dir, dir / ".tbuild" / "packages", first, diagnostics)) << diagnostics.diagnostics().dump();
    const auto marker = toolkit::File::read(first.sourceDir / ".tbuild_archive_marker");

    toolkit::ArchiveFetchResult second;
    ASSERT_TRUE(fetcher.fetchAndExtract(dep, dir, dir / ".tbuild" / "packages", second, diagnostics)) << diagnostics.diagnostics().dump();
    EXPECT_EQ(toolkit::File::read(second.sourceDir / ".tbuild_archive_marker"), marker);
    std::filesystem::remove_all(dir);
}
