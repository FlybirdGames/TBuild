...
namespace toolkit
{
                        ...
                        const std::vector<std::string> &cliOverrides)
    {
        ...
        const auto resolvedOutputDir = outputDir.empty()
                                           ? workspaceRoot / ".tpkg" / "generated" / "cmake"
        ...
        diagnostics.info("CMake consumers can use: find_package(tpkg CONFIG REQUIRED)");
        ...
        if (model.workspace.generateCMakeUserPresets)
        {
            ...
            if (File::exists(presetsPath))
            {
                ...
                diagnostics.info("add this cache variable to an existing preset if desired: CMAKE_PREFIX_PATH=${sourceDir}/.tpkg/generated/cmake");
            }
            else
            {
                ...
                preset["configurePresets"] = nlohmann::json::array({{
                    {"name", "tpkg-deps"},
                    {"displayName", "tpkg dependency prefix"},
                    {"hidden", true},
                    {"cacheVariables", {{"CMAKE_PREFIX_PATH", "${sourceDir}/.tpkg/generated/cmake"}}},
                }});
                std::string error;
                ...
                diagnostics.info("wrote optional CMakeUserPresets.json with hidden preset 'tpkg-deps'");
            }
        }
        return 0;
    }
