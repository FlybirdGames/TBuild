...
namespace
{
    ...
    TEST(LockFileTests, WritesAndReadsPackages)
    {
        ...
        const auto path = dir / "tbuild.lock.toml";
    ...
    TEST(LockFileTests, PackageCacheUsesShortSourceHashAndRefForRemoteDependencies)
    {
        ...
        toolkit::PackageCache cache(".tbuild/packages");
        const auto path = cache.source(dependency).generic_string();

        EXPECT_NE(path.find(".tbuild/packages/fmt/"), std::string::npos);
    ...
    TEST(LockFileTests, WritesAndReadsConfigToolchainFields)
    {
        ...
        toolkit::JsonDiagnosticSink diagnostics;
        ASSERT_TRUE(toolkit::LockFile::write(dir / "tbuild.lock.toml", lock, diagnostics)) << diagnostics.diagnostics().dump();

        toolkit::LockFile loaded;
        ASSERT_TRUE(toolkit::LockFile::read(dir / "tbuild.lock.toml", loaded, diagnostics)) << diagnostics.diagnostics().dump();
        ASSERT_EQ(loaded.packages.size(), 1u);
    ...
    TEST(LockFileTests, ArchiveHashAndPatchHashAffectBuildHash)
    {
        toolkit::DependencyDesc dependency;
