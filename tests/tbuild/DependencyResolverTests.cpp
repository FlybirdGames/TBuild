...
namespace
{
    ...
    TEST(DependencyResolverTests, RestoresLocalHeaderOnlyAndPrebuiltPackages)
    {
        ...
        EXPECT_TRUE(toolkit::File::exists(dir / "tbuild.lock.toml"));
        toolkit::LockFile lockFile;
        ASSERT_TRUE(toolkit::LockFile::read(dir / "tbuild.lock.toml", lockFile, diagnostics)) << diagnostics.diagnostics().dump();
        ...
        for (const auto &locked : lockFile.packages)
        {
            ...
            EXPECT_TRUE(toolkit::File::exists(dir / ".tbuild/artifacts" / locked.artifactId / "artifact.toml"));
        }
        EXPECT_EQ(toolkit::File::read(dir / "tbuild.lock.toml").find("E:"), std::string::npos);
        EXPECT_EQ(toolkit::File::read(dir / "tbuild.lock.toml").find("cache_path"), std::string::npos);
...
int value() {
    return 42;
        ...
        toolkit::PackageCache cache(dir / ".tbuild/packages");
        ...
        const auto artifactPath = dir / ".tbuild" / "artifacts" / locked.artifactId;
        ...
        const auto lockText = toolkit::File::read(dir / "tbuild.lock.toml");
        ...
        EXPECT_EQ(toolkit::File::read(dir / "tbuild.lock.toml"), lockText);
    ...
    TEST(DependencyResolverTests, FailedGitBuildReusesSourceCheckoutOnNextRestore)
    {
        const auto dir = makeTempDir();
    ...
    TEST(DependencyResolverTests, ArchiveDependencyRestoresHeaderOnlyPackage)
    {
        ...
        EXPECT_FALSE(locked.buildHash.empty());
        EXPECT_TRUE(toolkit::File::exists(dir / ".tbuild" / "artifacts" / locked.artifactId / "include" / "hello" / "hello.hpp"));
    ...
    TEST(DependencyResolverTests, RestoreLockedDoesNotRewriteLockfile)
    {
        ...
        const auto before = toolkit::File::read(dir / "tbuild.lock.toml");
        ...
        EXPECT_EQ(toolkit::File::read(dir / "tbuild.lock.toml"), before);
    ...
    TEST(DependencyResolverTests, UpdatePackageOnlyUpdatesSpecifiedPackage)
    {
        ...
        ASSERT_TRUE(toolkit::LockFile::read(dir / "tbuild.lock.toml", before, diagnostics)) << diagnostics.diagnostics().dump();
        ...
        ASSERT_TRUE(toolkit::LockFile::read(dir / "tbuild.lock.toml", after, diagnostics)) << diagnostics.diagnostics().dump();
