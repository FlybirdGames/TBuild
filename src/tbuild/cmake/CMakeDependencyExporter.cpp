...
namespace toolkit
{
    namespace
    {
        ...
        std::string targetName(const std::string &packageName)
        {
            ...
            return "tbuild::" + (result.empty() ? std::string("package") : result);
        }
        ...
        {
            config << R"cmake(
if(TBUILD_COMPILER_KIND MATCHES "^(msvc|clang-cl)$" OR TBUILD_TOOLCHAIN_ID MATCHES "msvc|clang-cl")
    if(NOT (MSVC OR CMAKE_CXX_COMPILER_ID MATCHES "MSVC|Clang" OR CMAKE_C_COMPILER_ID MATCHES "MSVC|Clang"))
        message(WARNING "tbuild artifacts were restored with an MSVC ABI toolchain (${TBUILD_TOOLCHAIN_ID}), but CMake may not be using a compatible compiler. CMAKE_CXX_COMPILER_ID=${CMAKE_CXX_COMPILER_ID}")
    endif()
endif()

if(TBUILD_COMPILER_KIND MATCHES "^(gcc|clang)$" AND TBUILD_TOOLCHAIN_ID MATCHES "mingw")
    if(MSVC OR CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
        message(FATAL_ERROR "tbuild artifacts were restored with a MinGW toolchain, but this CMake project is using MSVC")
    endif()
elseif(TBUILD_TOOLCHAIN_ID MATCHES "mingw")
    if(MSVC OR CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
        message(FATAL_ERROR "tbuild artifacts were restored with a MinGW toolchain, but this CMake project is using MSVC")
    endif()
endif()

if(TBUILD_CONFIG STREQUAL "debug")
    if(DEFINED CMAKE_BUILD_TYPE)
        if(CMAKE_BUILD_TYPE AND NOT CMAKE_BUILD_TYPE STREQUAL "Debug")
            message(FATAL_ERROR "tbuild debug artifacts cannot be used with CMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE}")
        endif()
    endif()
    if(DEFINED CMAKE_CONFIGURATION_TYPES)
        list(FIND CMAKE_CONFIGURATION_TYPES Debug _tbuild_debug_config_index)
        if(_tbuild_debug_config_index EQUAL -1)
            message(FATAL_ERROR "tbuild debug artifacts require a Debug configuration in this multi-config CMake project")
        endif()
    endif()
elseif(TBUILD_CONFIG STREQUAL "release")
    if(DEFINED CMAKE_BUILD_TYPE)
        if(CMAKE_BUILD_TYPE STREQUAL "Debug")
            message(FATAL_ERROR "tbuild release artifacts cannot be used with CMAKE_BUILD_TYPE=Debug")
        endif()
    endif()
    if(DEFINED CMAKE_CONFIGURATION_TYPES)
        list(LENGTH CMAKE_CONFIGURATION_TYPES _tbuild_config_count)
        if(_tbuild_config_count EQUAL 1)
            list(GET CMAKE_CONFIGURATION_TYPES 0 _tbuild_only_config)
            if(_tbuild_only_config STREQUAL "Debug")
                message(FATAL_ERROR "tbuild release artifacts cannot be used with a Debug-only multi-config CMake project")
            endif()
        endif()
    endif()
endif()
...
if(TBUILD_ARCH)
    string(TOLOWER "${CMAKE_SYSTEM_PROCESSOR}" _tbuild_consumer_arch)
    if(TBUILD_ARCH STREQUAL "x64" AND _tbuild_consumer_arch AND NOT _tbuild_consumer_arch MATCHES "x86_64|amd64")
        message(FATAL_ERROR "tbuild artifact arch is x64 but CMake reports ${CMAKE_SYSTEM_PROCESSOR}")
    endif()
    if(TBUILD_ARCH STREQUAL "x86" AND _tbuild_consumer_arch AND NOT _tbuild_consumer_arch MATCHES "x86|i.86")
        message(FATAL_ERROR "tbuild artifact arch is x86 but CMake reports ${CMAKE_SYSTEM_PROCESSOR}")
    endif()
    if(TBUILD_ARCH STREQUAL "arm64" AND _tbuild_consumer_arch AND NOT _tbuild_consumer_arch MATCHES "aarch64|arm64")
        message(FATAL_ERROR "tbuild artifact arch is arm64 but CMake reports ${CMAKE_SYSTEM_PROCESSOR}")
    endif()
endif()

if(MSVC AND TBUILD_RUNTIME MATCHES "static|dynamic")
    if(DEFINED CMAKE_MSVC_RUNTIME_LIBRARY)
        if(CMAKE_MSVC_RUNTIME_LIBRARY)
            if(TBUILD_RUNTIME STREQUAL "static" AND CMAKE_MSVC_RUNTIME_LIBRARY MATCHES "DLL")
                message(FATAL_ERROR "tbuild artifacts use static MSVC runtime, but CMAKE_MSVC_RUNTIME_LIBRARY=${CMAKE_MSVC_RUNTIME_LIBRARY}")
            endif()
            if(TBUILD_RUNTIME STREQUAL "dynamic" AND NOT CMAKE_MSVC_RUNTIME_LIBRARY MATCHES "DLL")
                message(FATAL_ERROR "tbuild artifacts use dynamic MSVC runtime, but CMAKE_MSVC_RUNTIME_LIBRARY=${CMAKE_MSVC_RUNTIME_LIBRARY}")
            endif()
        endif()
    endif()
endif()
                                   ...
                                   DiagnosticSink &diagnostics)
    {
        ...
        targets << "# Generated by ToolkitBuild. Do not edit.\n\n";
        ...
        if (!File::write(outputDir / "tbuildTargets.cmake", targets.str(), &error))
        {
        ...
        config << "# Generated by ToolkitBuild. Do not edit.\n\n";
        config << "include_guard(GLOBAL)\n";
        config << cmakeSet("TBUILD_TOOLCHAIN_ID", metadata.toolchainId);
        config << cmakeSet("TBUILD_COMPILER_KIND", metadata.compilerKind);
        config << cmakeSet("TBUILD_CONFIG", metadata.config);
        config << cmakeSet("TBUILD_ARCH", metadata.arch);
        config << cmakeSet("TBUILD_RUNTIME", metadata.runtime);
        config << cmakeSet("TBUILD_LINKAGE", metadata.linkage);
        writeAbiChecks(config);
        config << "include(\"${CMAKE_CURRENT_LIST_DIR}/tbuildTargets.cmake\")\n";
        if (!File::write(outputDir / "tbuildConfig.cmake", config.str(), &error))
        {
            diagnostics.error("failed to write tbuildConfig.cmake: " + error);
            return false;
        }
        return true;
    }
