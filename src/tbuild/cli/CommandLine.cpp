...
#include "tbuild/cli/CommandLine.hpp"
...
namespace toolkit
{
#ifndef TBUILD_VERSION
#define TBUILD_VERSION "0.0.0"
#endif

    std::string CmdLine::version()
    {
        return fmt::format("{}-{}-{}-{}", TBUILD_VERSION, Environment::buildModeName(), Environment::compilerName(), Environment::hostPlatformName());
    }
    ...
    {
        CLI::App app{"ToolkitBuild C/C++ dependency manager"};
        app.set_version_flag("--version", CmdLine::version());

        CommandContext context;
        app.add_option("--root", context.rootOption, "Workspace root containing tbuild.deps.lua")
            ->capture_default_str();

#ifndef NDEBUG
        auto *doctor = app.add_subcommand("doctor", "Check the local ToolkitBuild runtime");
        ...
        app.add_subcommand("load", "Load tbuild.deps.lua and validate the dependency DSL")->callback([&]
                                                                                                     { context.exitCode = Commands::load(root(context.rootOption)); });
        app.add_subcommand("dump-model", "Load tbuild.deps.lua and print the dependency model JSON")->callback([&]
                                                                                                               { context.exitCode = Commands::dumpModel(root(context.rootOption)); });
#endif
        app.add_subcommand("deps", "Print dependencies declared in tbuild.deps.lua")->callback([&]
        ...
        generate->add_option("--output", context.generateOutputDir, "Output directory for generated CMake files")->default_val(".tbuild/generated/cmake");
        ...
        auto *clean = app.add_subcommand("clean", "Clean tbuild dependency cache and generated integration files");
        clean->add_option("package", context.cleanPackage, "Only clean one dependency")->required(false);
        clean->add_flag("--all", context.cleanAll, "Remove the whole .tbuild directory");
        clean->add_flag("--artifacts", context.cleanArtifacts, "Also remove .tbuild/artifacts");
        clean->add_flag("--sources", context.cleanSources, "Also remove .tbuild/packages source cache");
        clean->add_flag("--lock", context.cleanLock, "Also remove tbuild.lock.toml");
        clean->callback([&]
        ...
        sdk->add_subcommand("detect", "Detect SDK/toolchains and write .tbuild/toolchains/host.toml")->callback([&]
        ...
        auto *sdkSelect = sdk->add_subcommand("select", "Save preferred local toolchain in .tbuild/local.toml");
