...
namespace
{
    ...
    TEST(CMakeDependencyExporterTests, WritesFindPackageConfigAndImportedTargets)
    {
        ...
        const auto output = root / ".tbuild/generated/cmake";
        ...
        const auto config = toolkit::File::read(output / "tbuildConfig.cmake");
        EXPECT_NE(config.find("tbuildTargets.cmake"), std::string::npos);

        const auto targets = toolkit::File::read(output / "tbuildTargets.cmake");
        EXPECT_NE(targets.find("add_library(tbuild::fmt INTERFACE IMPORTED)"), std::string::npos);
        ...
        EXPECT_NE(config.find("TBUILD_CONFIG"), std::string::npos);
        EXPECT_NE(config.find("TBUILD_COMPILER_KIND"), std::string::npos);
